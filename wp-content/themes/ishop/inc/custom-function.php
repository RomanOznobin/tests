...
function ishop_breadcrumbs() {
    ...
    if (is_category()) {
        ...
        echo $before . __('Archive by category "' . single_cat_title('', false) . '"', 'ishop') . $after;
    } elseif (is_day()) {
        ...
        echo $before . __('Search results for "' . get_search_query() . '"','ishop') . $after;
    } elseif (is_tag()) {
        echo $before . __('Posts tagged "' . single_tag_title('', false) . '"','ishop') . $after;
    } elseif (is_author()) {
        global $author;
...
function ishop_comment_form_fields( $fields ) {
    ...
    $fields   =  array(
        'author' => '<div class="large-6 columns"><div class="row collapse prefix-radius"><div class="small-3 columns">' . '<span class="prefix"><i class="fa fa-user"></i>' . __( 'Name','ishop' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</span> </div>' .
        ...
        'email'  => '<div class="large-6 columns"><div class="row collapse prefix-radius"><div class="small-3 columns">' . '<span class="prefix"><i class="fa fa-envelope-o"></i>' . __( 'Email','ishop' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</span></div> ' .
        ...
        'url'    => '<div class="large-6 columns"><div class="row collapse prefix-radius"><div class="small-3 columns">' . '<span class="prefix"><i class="fa fa-external-link"></i>' . __( 'Website','ishop' ) . '</span> </div>' .
...
function ishop_search_form( $form ) {
	$form = '<form role="search" method="get" id="searchform" class="searchform" action="' . esc_url(home_url( '/' )) . '" >
           ...
           <i class="fa fa-search"></i><input type="submit" class="button postfix" value="'. esc_attr__( 'Go','ishop' ) .'" />
            </div>
            </div>
            </div>
            </div>  
...
add_filter( 'woocommerce_add_to_cart_fragments', 'ishop_woocommerce_header_add_to_cart_fragment' );
   ...
   function ishop_wooaccinfo(){
	...
	echo '" title="'.__('My Account','ishop').'">'.__('My Account','ishop').'</a></li>';
	}
else {

	echo '<li><a class="myacclo" href="';
	echo esc_url(get_permalink( get_option('woocommerce_myaccount_page_id')) ); 
	echo '" title="'.__('Login / Register','ishop').'">'.__('Login / Register','ishop').'</a></li>';
	}
	global $woocommerce;
...
function ishop_after_shop_item_buttons() {
             ...
             echo sprintf( '<a href="%s" class="show_details_button"%s>%s</a></div></div>', esc_url(get_permalink()), $styles, __( 'Details', 'ishop' ) );
     }
...
function ishop_close_summary_div()
{
  ...
  <a href="mailto:?Subject=Simple Share Buttons&amp;Body=I%20saw%20this%20and%20thought%20of%20you!%20' . esc_url($currentlink) .'" class="item instagram">
...
if (get_theme_mod('woocommerce_share_buttons')==''){
...
function ishop_css(){
	$custom_css = '
	#masthead > div.header-area{background-image:url("'.esc_url(get_theme_mod('header_image')).'"); display: flex;}
