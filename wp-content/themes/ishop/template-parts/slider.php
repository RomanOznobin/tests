...
if(get_theme_mod('hide_slider')!==''){
    echo '<ul class="" data-orbit data-orbit data-options="animation_speed:500; bullets:false; slide_number: false; pause_on_hover: false;timer_speed: 5000;">';
     ...
     $i = 1;
        ...
        {
          echo '<li>';
        echo ' <img src="'. esc_url(get_theme_mod('slide_image'.$i) ). '" alt="'.esc_attr(get_theme_mod('slide_caption'.$i)).'" /><div class="orbit-caption">'.esc_html(get_theme_mod('slide_caption'.$i)).'</div>';
        echo "</li>";
        $i++;
        } 
        echo '</ul>';
