...
/* 
 * This file contain code for iShop widget.
 * 
 * @package ishop
 */
...
<div class="tabs-content">
  <section role="tabpanel" aria-hidden="false" class="content active" id="panel2-1">
	<?php 
		$ishop_popularposts = array( 
			'ignore_sticky_posts' => true,
			'showposts' => get_theme_mod("ishop_widget_range"),
			'orderby' => 'comment_count',
							);
		$the_query = new WP_Query( $ishop_popularposts );
		if ( $the_query->have_posts() ) :
		while ( $the_query->have_posts() ) : $the_query->the_post();
	?>

	<div class="sidebarwidget1">
		<?php if ( has_post_thumbnail() ) {the_post_thumbnail('ishop_themewidget');} else { ?>
  ...
  <section role="tabpanel" aria-hidden="true" class="content" id="panel2-2">
	<?php 
		$ishop_recentposts = array( 
			'ignore_sticky_posts' => true,
			'showposts' => get_theme_mod("ishop_widget_range"),
			'orderby' => 'date',
							);
		$the_query = new WP_Query( $ishop_recentposts );
		if ( $the_query->have_posts() ) :
		while ( $the_query->have_posts() ) : $the_query->the_post();
	?>
	...
	<div class="sidebarwidget1">
		...
		<?php } ?> 
		<a title="<?php the_title(); ?>" href="<?php the_permalink() ?>" rel="bookmark"><?php the_title(); ?></a>
                <br /><div class="widgetinfo"><?php ishop_posted_on(); ?></div>
		<div class="clear"></div>
		</div>			
		<?php endwhile;  endif;  wp_reset_postdata(); ?>
  </section>
